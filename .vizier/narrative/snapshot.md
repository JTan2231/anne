...
- `anne review --base <base> --head <head>` and `anne review <base>...<head>` both normalize to triple-dot review semantics. The command resolves `git merge-base <base> <head>` and reviews the diff from that merge base to `<head>`.
...
- Findings themselves do not make the command fail in v1. Operational problems such as git resolution failures, agent failures, invalid JSON, or invalid anchors mark the run failed while preserving the bundle for audit/debugging.
- Automated coverage exists for CLI parsing, hunk-anchor parsing, merge-base review behavior, plain-text agent execution, and progress-filter execution with invalid-anchor failure handling.
