...
case \"${subcommand}\" in
  ...
  build)
    ...
    cat >\"${target_dir}/release/anne\" <<'EOF'
#!/bin/sh
printf '%s\\n' 'anne stub'
EOF
    chmod +x \"${target_dir}/release/anne\"
    ;;
  *)
...
#[test]
fn install_sh_stages_and_uninstalls() -> TestResult {
    let tmp = TempDir::new()?;
    ...
    let expected_exe = stage.join("usr/local/bin/anne");
    let expected_agents = [
        "usr/local/share/anne/agents/default/agent.sh",
        "usr/local/share/anne/agents/default/filter.sh",
    ];
    let expected_manifest = stage.join("usr/local/share/anne/install-manifest.txt");
    ...
    assert!(
        manifest_lines.contains("/usr/local/bin/anne"),
        "manifest missing anne binary: {manifest}"
    );
    for rel in expected_agents {
        let manifest_path = format!("/{rel}");
        assert!(
            manifest_lines.contains(manifest_path.as_str()),
            "manifest missing agent shim entry {manifest_path}: {manifest}"
        );
    }
    ...
    assert!(
        cargo_invocations.contains("build --locked --release -p anne"),
...
#[test]
fn install_sh_dry_run_writes_nothing() -> TestResult {
    let tmp = TempDir::new()?;
    let root = tmp.path().join("src");
    fs::create_dir_all(&root)?;
    seed_install_fixture_root(&root)?;
    ...
    assert!(
        ...
        "expected dry-run to avoid generating Cargo.lock"
    );
    Ok(())
}
...
#[test]
#[cfg(unix)]
fn install_sh_requires_writable_prefix() -> TestResult {
    if is_root_user() {
        return Ok(());
    }
...
#[test]
#[cfg(unix)]
fn install_sh_root_defaults_to_temp_cargo_target_dir() -> TestResult {
    let tmp = TempDir::new()?;
    let root = tmp.path().join("src");
    fs::create_dir_all(&root)?;
    seed_install_fixture_root(&root)?;
