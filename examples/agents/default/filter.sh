...
# default/filter.sh
# -----------------
# This translates the Codex JSONL event stream into progress on stderr and a
# single final assistant reply on stdout.

USE_COLOR=0
if [ -t 2 ] && [ -z "${NO_COLOR:-}" ] && [ -z "${ANNE_NO_ANSI:-}" ]; then
  USE_COLOR=1
fi

if ! command -v jq >/dev/null 2>&1; then
  printf '[default-filter] jq is required on PATH to render progress\n' >&2
  exit 1
fi
